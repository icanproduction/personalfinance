...
export default function BudgetPage() {
  ...
  const loadBudgets = async () => {
    ...
    const res = await fetch(`/api/budgets/status?period=${period}&account_type=${accountTypesParam}`)
  ...
  return (
    <div className="px-4 pt-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-xs text-slate-400 font-medium">{mode === 'personal' ? 'Personal' : 'Business'}</h2>
        <ModeToggle />
      </div>
